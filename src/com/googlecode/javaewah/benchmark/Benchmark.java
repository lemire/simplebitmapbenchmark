                ...
                for (int sparsity = 1; sparsity < 30 - nbr; sparsity += 2) {
                        ...
                        for (int k = 0; k < N; ++k) {
                                ...
                                IteratingRLW ewahor = IteratorAggregation
                                        .bufferedor(ewahcp);
                                EWAHCompressedBitmap ewahorp = EWAHCompressedBitmap
                                        .or(Arrays.copyOf(ewah, k + 1));
                                EWAHCompressedBitmap mewahor = IteratorUtil
                                        .materialize(ewahor);
                                if (!ewahorp.equals(mewahor))
                        ...
                        for (int r = 0; r < repeat; ++r)
                                for (int k = 0; k < N; ++k) {
                                        ...
                                        IteratingRLW ewahor = IteratorAggregation
                                                .bufferedor(ewahcp);
                                        bogus += IteratorUtil.materialize(
                                                ewahor).sizeInBits();
                                }
                        aft = System.currentTimeMillis();
                        ...
                        for (int k = 0; k < N; ++k) {
                                ...
                                IteratingRLW ewahand = IteratorAggregation
                                        .bufferedand(ewahcp);
                                EWAHCompressedBitmap ewahandp = EWAHCompressedBitmap
                                        .and(Arrays.copyOf(ewah, k + 1));
                                EWAHCompressedBitmap mewahand = IteratorUtil
                                        .materialize(ewahand);
                                if (!ewahandp.equals(mewahand))
                                        throw new RuntimeException("bug");
                        }
                        // fast logical and
                        bef = System.currentTimeMillis();
